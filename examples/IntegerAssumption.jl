...
# Create requires spaces
space = [custom_color(indexes_R[i], indexes_G[i], indexes_B[i]) for i = 1:N]
space2 = deepcopy(space)
original_space = [custom_color(indexes_R[i], indexes_G[i], indexes_B[i]) for i = 1:N]

# Optimize the spaces
optimize_space_int!(space, epss)
optimize_space_int!(space2, epss, :yes)

# Chekc the differences
differences = Float64[]
for i = 1:N
    append!(differences, round(space[i].r) - space2[i].r)
    append!(differences, round(space[i].g) - space2[i].g)
    append!(differences, round(space[i].b) - space2[i].b)
end

# Plot the differences
Plots.bar(countmap(differences), bar_width=0.5)
xlabel!("Change from original color")
ylabel!("Number of occurences", legend=false, fontfamily="Computer Modern")
Plots.savefig("../../final_report/figures/integer.pdf")
